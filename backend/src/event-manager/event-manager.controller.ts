...
export class EventManagerController {
  ...
  async findAll(
    ...
    return this.eventManagerService.findAll(sortBy, sortOrder);
  }
  ...
  @Get('user/:userId')
  async getEventManagerByUserId(
    @Param('userId') userId: number,
  ): Promise<EventManager> {
    const eventManager =
      await this.eventManagerService.getEventManagerByUserId(userId);
    return eventManager;
  }

  @Get('profile-picture/:eventManagerId')
  @Roles(UserRole.ADMIN, UserRole.EVENT_MANAGER)
  async getProfilePicture(@Param('eventManagerId') eventManagerId: number) {
    const profileData =
      await this.eventManagerService.getProfilePicture(eventManagerId);
    ...
    return profileData; // Only return the profile picture data
  }
  ...
  // add volunteer and sponsor by event manager
