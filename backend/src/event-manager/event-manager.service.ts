...
export class EventManagerService {
  ...
  // async findAll(sortBy: string = 'id', sortOrder: 'ASC' | 'DESC' = 'ASC'): Promise<EventManager[]> {
  //   return this.eventManagerRepository.find({ 
  //     relations: ['user'],
  //     order: { [sortBy]: sortOrder },
  //    });
  // }

  async findAll(sortBy: string = 'id', sortOrder: 'ASC' | 'DESC' = 'ASC'): Promise<any[]> {
    const eventManagers = await this.eventManagerRepository.find({
      relations: ['user'],
      order: { [sortBy]: sortOrder },
    });

    return eventManagers.map(em => ({
      ...em,
      userId: em.user.id,
      user: undefined // Remove the user object to avoid sending unnecessary data
    }));
  }
  ...
  async getProfilePicture(eventManagerId: number): Promise<{ profilePicture: string }> {
    const eventManager = await this.eventManagerRepository.findOne({
        where: { id: eventManagerId },
        select: ['profilePicture'], // Only select the profilePicture field
    });
    ...
    return { profilePicture: eventManager.profilePicture }; // Return only profilePicture
  }

  async getEventManagerByUserId(userId: number): Promise<EventManager> {
    const eventManager = await this.eventManagerRepository
      .createQueryBuilder('eventManager')
      .innerJoinAndSelect('eventManager.user', 'user')
      .where('user.id = :userId', { userId })
      .getOne();

    if (!eventManager) {
      throw new NotFoundException(`EventManager with User ID ${userId} not found`);
    }
